<form action="{{ $action }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        <input type="datetime-local" name="start_time" id="start_time" class="form-control"
            value="{{ old('start_time', optional(optional($overtime)->start_time)->format('Y-m-d\TH:i')) }}" required>
    </div>
    ...
    <div class="form-group">
        ...
        <input type="datetime-local" name="end_time" id="end_time" class="form-control"
            value="{{ old('end_time', optional(optional($overtime)->end_time)->format('Y-m-d\TH:i')) }}" required>
    ...
    <div class="form-group">
        ...
        <select name="multiplier" id="multiplier" class="form-control" required>
            <option value="1.5" {{ old('multiplier', $overtime->multiplier ?? '') == 1.5 ? 'selected' : '' }}>Normal Day (1.5x)</option>
            <option value="2.0" {{ old('multiplier', $overtime->multiplier ?? '') == 2.0 ? 'selected' : '' }}>Rest Day (2.0x)</option>
            <option value="3.0" {{ old('multiplier', $overtime->multiplier ?? '') == 3.0 ? 'selected' : '' }}>Public Holiday (3.0x)</option>
        </select>
    </div>
    ...
    <div class="form-group">
        ...
        <input class="form-control" type="number" name="hourly_rate" id="hourly_rate"
            value="{{ old('hourly_rate', $hourlyRate) }}" readonly>
    </div>
