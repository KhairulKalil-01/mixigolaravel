                    ...
                    <div class="col-12 col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <h4>My Salary Advance Requests</h4>
                            </div>
                            <div class="card-body">
...
@section('scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            @can('Create Staff Advance')
                buttons.push({
                    text: "Create New Advance Request",
                    attr: {
                        title: "Create",
                        id: "create"
                    },
                    ...
                    action: function(e, dt, node, config) {
                        window.location.href = "{{ route('staff-salary-advances.create') }}";
                    }
                });
            @endcan
            ...
            var table = $(table_advance).DataTable({
                ...
                bDeferRender: true,
                ajax: {
                    url: "{{ route('staff-salary-advances.fetch') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                    }
                ...
                columnDefs: [{
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        @can('View Staff Advance')
                            buttons +=
                                `<a href="/staff-salary-advances/${row.id}" class="btn btn-info viewBtn">View</a>&nbsp;`;
                        @endcan
                        /* No edit for staff salary advance (Only create/delete) */
                        @can('Delete Staff Advance')
                            buttons +=
